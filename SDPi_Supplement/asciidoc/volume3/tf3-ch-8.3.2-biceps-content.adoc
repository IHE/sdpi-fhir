...
.^| (tba)
.^| [[acronym_dgw,DGW]] DGW, [[acronym_agw,AGW]] AGW
.^| Gateway PARTICIPANT that provides, for example, ADT information or laboratory results (inbound). Note:  AGW is for an Alert Gateway system.  (For example, see <<vol1_clause_appendix_c_use_case_acns_technical_precondition>>)
