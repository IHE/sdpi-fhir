...
.^| Gateway PARTICIPANT that provides, for example, ADT information or laboratory results (inbound). Note:  <<acronym_agw>> is for an Alert Gateway system.  (For example, see <<vol1_clause_appendix_c_use_case_acns_technical_precondition>>)
...
#PRIORITY-1 -- Include mixed safety class language + links to boilerplate SES sections for Semantic Content#
