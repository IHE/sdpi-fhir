...
The <<acronym_biceps>> standard, <<ref_ieee_11073_10207_2017>>, provides an extensive semantic model for all information exchanged between <<actor_somds_participant>> systems.  This section provides a general background for <<acronym_biceps>>-based content, including both what is unique to this standard (e.g., different from the <<term_classic_dim>>), and any extensions that are made by this SDPi supplement.


.R0701
[sdpi_requirement#r0701,sdpi_req_level=shall]
****
All <<actor_somds_participant>> systems shall fully implement the semantic content requirements in the <<ref_ieee_11073_10207_2017>> <<acronym_biceps>> standard.
...
Depending on a systems capabilities, they may be configured -- statically or dynamically -- with context information such as location or patient identification, or they may discover a <<actor_somds_provider>> on the network from which they can retrieve the information.
...
Additionally, only limited support for the *_AlertSystem_* (and related objects), and for the *_SystemContext_* types are provided in SDPi 1.0.
...
.^| [[system_type_central,Central]] Central
.^| (tba)
.^|
...
.^| (tba)
.^|
.^| A system that discovers and integrates or "aggregates" information from one or more devices and makes them accessible from a single <<actor_somds_provider>> <<acronym_mdib>>. Aggregated devices may be <<actor_somds_participant>>s that support <<acronym_sdc>> connectivity or may be interfaced to the aggregator system by some other non-standardized means (e.g., analog sensors or proprietary protocols).

.^| [[system_type_dashboard,Dashboard]] Dashboard
.^| (tba)
.^|
.^| A system that displays information from one or more <<actor_somds_participant>> systems associated with a single patient.  Similar to a _Cockpit_ but without device-external control capabilities.  May include both metric and alert information.

.^| [[system_type_device_gateway,Gateway]] Gateway
.^| (tba)
.^| [[acronym_dgw,DGW]] DGW, {var_label_system_type_acronym_agw}
.^| Gateway PARTICIPANT that provides, for example, ADT information or laboratory results (inbound). Note:  {var_label_system_type_acronym_agw} is for an Alert Gateway system.  (For example, see <<vol1_clause_appendix_c_use_case_acns_technical_precondition>>)
...
[cols="1"]
|===
a| *SDPi Supplement Version Note*:  The <<acronym_sdc>> standards community is evaluating the use of safety class elements in the BICEPS specification (see <<ref_ieee_11073_10207_2017>>), and the related https://github.com/IHE/sdpi-fhir/issues/61[Github Issue #61 _Topic: SDPi-xC with Mixed Device Safety Classes_].  The result of that discussion will directly impact the BICEPS SES Considerations section below.
...
include::biceps-content-module/tf3-ch-8.3.2.7-applicable-productions-specification-type-codes.adoc[]

include::biceps-content-module/tf3-ch-8.3.2.7-applicable-attribute-type-codes.adoc[]

include::biceps-extension-provisions/tf3-ch-8.3.2.7-extension-coded-attribute.adoc[]

// 8.3.2.8
===== BICEPS Sex and Gender Extensions
// This content will also be referenced from TF-2 Appendix B DEC Gateway
include::biceps-extension-provisions/tf3-ch-8.3.2.8-extension-gender.adoc[]
