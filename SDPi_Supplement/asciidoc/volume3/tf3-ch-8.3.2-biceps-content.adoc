...
.<<acronym_mdc>> Nomenclature System Type Extensions
...
.^| (tba)
.^|
.^| Discovering and integrates devices to an SDC network aggregator; systems may be <<actor_somds_participant>>s that support <<acronym_sdc>> connectivity or may be connected to the aggregator system by some other non-standardized means.
