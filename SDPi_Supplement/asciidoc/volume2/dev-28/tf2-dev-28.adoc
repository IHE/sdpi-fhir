...
The {var_label_dev_28_message_contextreport} message is sent whenever a <<actor_somds_consumer>> is subscribed to a <<actor_somds_provider>> and intends to get notified on certain changes of a <<actor_somds_provider>>.

[#vol2_clause_dev_28_message_notification_semantics]
====== Message Semantics

The {var_label_dev_28_message_contextreport} message does not contain any further semantics.

[#vol2_clause_dev_28_message_contextreport_expected_actions]
====== Expected Actions

When a <<actor_somds_provider>> sends this message, the receiving <<actor_somds_consumer>> could unsubscribe.
