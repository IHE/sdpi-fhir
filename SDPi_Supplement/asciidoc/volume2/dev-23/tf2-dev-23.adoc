...
|Announces its presence by broadcasting <<vol2_clause_dev_23_message_hello, {var_label_dev_23_message_hello}>> messages to all listening systems.
...
[#vol2_clause_dev_23_message_announce_network_presence_recurring_hello]
====== Recurring Hello
#TODO - THE FOLLOWING PARAGRAPH NEEDS SIGNIFICANT IMPROVEMENTS: introduce need for recurring hello message. Also decide if only upon reconnections (WHY?)#

#Whenever a <<actor_somds_provider>> detects reconnection to the network it can send a Hello message and . In order to prevent all the <<actor_somds_provider>>s from sending Hello messages at the same time, a random interval between 60 seconds and 120 seconds shall be kept before each retry after the network connection is restored. This will prevent congestion and decrease the likelihood of missed discovery messages.#

R2001
[sdpi_requirement#r2001,sdpi_req_level=shall]
****
A <<actor_somds_provider>> shall send recurring Hello messages after reconnection.
****

#NOTE that the sub-section below is transport dependent - shouldn't it be moved to the TF2 MDPWS annex?#
[#vol2_clause_dev_23_message_announce_network_presence_hello_message_size]
====== Hello message size
...
Moreover, as further mitigation measure, note that SOAP-over-UDP #[REF]# already defines a non-normative retry and back-off algorithm.
