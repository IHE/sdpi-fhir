...
//reset|+1|off
[#vol2_clause_dev_23,sdpi_offset=3]
...
|Announces its presence by broadcasting <<message_announce_network_presence>> messages to all listening systems.

|<<actor_somds_consumer>>
|Listens for <<message_announce_network_presence>> messages  to identify any provider systems that it may want to establish a connection with and further discover a provider’s service capabilities.
...
[#message_announce_network_presence]
===== {var_label_dev_23_message_announce_network_presence}

The {var_label_dev_23_message_announce_network_presence} message is part of the BICEPS _implicit discovery_ protocol for allowing <<actor_somds_consumer>>s to receive a notification when a <<actor_somds_provider>> is ready to exchange messages with other <<actor_somds_consumer>>s.
...
[#message_announce_network_presence_trigger_events]
====== Trigger Events
...
[#message_announce_network_presence_message_semantics,sdpi_depth=+1]
====== Message Semantics
