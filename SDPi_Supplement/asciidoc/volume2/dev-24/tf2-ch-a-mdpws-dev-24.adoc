...
* <<ref_ws_discovery_2009>> {var_uri_ws_discovery_probe}[Section 5.2 Probe]
* <<ref_dpws_2009>> {var_uri_dpws_discovery}[Section 3 Discovery]
...
====== Recurring Probe
While the number of Hello messages scales linearly with the number of <<actor_somds_provider>>s, in the worst case the number of Probe / ProbeMatch messages is the product of the number of <<actor_somds_provider>>s and the number of <<actor_somds_consumer>>s.
...
In addition, a filter criterion for Probe messages could be set.
For example, only <<actor_somds_consumer>>s that are assigned to a patient and lost network connection can send Probe messages.


.R7000
[sdpi_requirement#r7000,sdpi_req_level=shall]
****
A <<actor_somds_provider>> shall implement the scope matching rule `{var_uri_matching_rule}` as specified by <<ref_ws_discovery_2009>> in a way that the use of _prefix_ refers to the definition of a prefix in formal theory.
...
* <<ref_ws_discovery_2009>> {var_uri_ws_discovery_probe_match}[Section 5.3 Probe Match]
* <<ref_dpws_2009>> {var_uri_dpws_discovery}[Section 3 Discovery]
...
* <<ref_ws_discovery_2009>> {var_uri_ws_discovery_resolve}[Section 6.2 Resolve]
* <<ref_dpws_2009>> {var_uri_dpws_discovery}[Section 3 Discovery]
...
* <<ref_ws_discovery_2009>> {var_uri_ws_discovery_resolve_match}[Section 6.3 Resolve Match]
* <<ref_dpws_2009>> {var_uri_dpws_discovery}[Section 3 Discovery]
