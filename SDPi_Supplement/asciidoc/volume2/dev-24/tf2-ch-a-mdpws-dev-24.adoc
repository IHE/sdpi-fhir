...
`s12:Envelope/s12:Body/wsd:ProbeMatches`:: In case where multiple <<actor_somds_provider>>s are running on a single machine, `wsd:ProbeMatches` can contain multiple `wsd:ProbeMatch` results.
`s12:Envelope/s12:Body/wsd:ProbeMatches/wsd:ProbeMatch/wsa:EndpointReference/wsa:Address`:: The <<actor_somds_provider>>'s <<property_somds_provider_uid>> encoded as URI.
...
`s12:Envelope/s12:Body/wsd:ProbeMatches/wsd:Scopes`:: The <<property_discovery_scope>> of the <<actor_somds_provider>>, encoded as a list of URIs.
...
[source#vol2_clause_appendix_a_mdpws_dev_24_resolve_match,xml,linenums]
----
...
`s12:Envelope/s12:Body/wsd:ResolveMatches/wsd:ResolveMatch/wsa:EndpointReference/wsa:Address`:: The <<actor_somds_provider>>'s <<property_somds_provider_uid>> encoded as URI.
...
`s12:Envelope/s12:Body/wsd:ResolveMatches/wsd:Scopes`:: The <<property_discovery_scope>> of the <<actor_somds_provider>>, encoded as a list of URIs.
