...
|While a subscription is running, the <<actor_somds_provider>> delivers <<vol2_clause_dev_29_message_notification, {var_label_dev_29_message_notification}>> messages.
...
[plantuml#vol2_figure_dev_29_sequence, target=puml-dev-29-sequence, format=svg, reftext='{figure-caption} {counter:refnum}',width=100%]
....
...
The {var_label_dev_29_message_notification} messages contain updated reports and is delivered by a <<actor_somds_provider>> to subscribed <<actor_somds_consumer>>s.

[#vol2_clause_dev_29_message_notification_trigger_events]
====== Trigger Events

The {var_label_dev_29_message_notification} message is sent whenever a <<actor_somds_consumer>> is subscribed to a <<actor_somds_provider>> and intends to get notified on certain changes of a <<actor_somds_provider>>.
...
When a <<actor_somds_provider>> sends a {var_label_dev_29_message_notification} message, the receiving <<actor_somds_consumer>> could unsubscribe.
