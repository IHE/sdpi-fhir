...
The HL7 Patient Identification (PID) segment requires a mapping from the SDC patient context information to the PID segment field.

.Prerequisite of Valid Patient Context
[NOTE#ref_valid_pt_ctxt_note]
====
The SDC patient context information shall only be mapped to the corresponding fields in the HL7 PID segment when the requirements (e. g. inferred patient ensemble context, associated patient context, etc.) as defined in the *{var_sdc_std_10700}* are fulfilled.

If the SDC patient context information shall not be used for the mapping, please refer to the *{var_ihe_tf-2_short}* on how to populate the fields of the PID segment in this case.
====
...
Please refer also to the corresponding section in the *{var_ihe_tf-2_short}*.
...
The PID-8 field shall contain the administrative sex - usually the sex at birth. The table <<ref_tbl_pid8_mapping>> defines the mapping of the SDC patient's sex information to the data fields of the HL7 data type *IS* used in the PID-8 field.
