...
|If *pm:AbstractMetricDescriptor+++<wbr/>+++/@MetricAvailability* is set to "*Cont*", the field is set to start time of the *msg:PeriodicMetricReport* event. The *@DeterminationTime* of the continuously measured metrics to be exported in the same export interval has to be within the start time of the *msg:PeriodicMetricReport* event and the *msg:RetrievabilityInfo+++<wbr/>+++/@UpdatePeriod*. Episodic metrics are exported in the same HL7 message, when their *@DeterminationTime* is equal or newer than the start time defined in the OBX-7 field.
