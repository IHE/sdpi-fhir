...
==== Scope
This chapter defines the mapping from <<acronym_mdib>> content as defined in this document and its underlying standards, to IHE Device Enterprise Communication (DEC) profile messages as defined in the <<ihe_pcd_tf_2_2019>>.

The SDPi gateway actor represents the Device Observation Reporter (DOR) role of the IHE DEC profile.

The following sections supplement the IHE DEC profile as appropriate. If there are no supplementing definitions, the definitions as described in the <<ihe_pcd_tf_2_2019>> will apply.
...
The following sections refer to the *Appendix B Common Segment Descriptions* of the <<ihe_pcd_tf_2_2019>>.
...
[%collapsible]
====
NOTE: For further information, please refer to the <<ihe_pcd_tf_2_2019>>.
====
****
...
[%collapsible]
====
NOTE: <<ref_tbl_dec_obr7_mapping>> defines the mapping of the SDC metric measurement timestamp to the data fields of the HL7 data type *DTM* used in the OBR-7 field.
====
****
...
For episodic measurements and the absence of any continuously measured metrics for this export interval, a <<actor_somds_dec_gateway>> shall set the OBR-7 field to the oldest measurement timestamp of the episodic metrics to be exported in this interval.
...
More information about the containment tree mapping can be found in *Appendix A Mapping ISO/IEEE 11073 Domain Information Model to HL7* in <<ihe_pcd_tf_2_2019>>.
...
[%collapsible]
====
NOTE: Please refer to the IHE technical framework <<ihe_pcd_tf_2_2019>> for further information.
====
****
...
****
A <<actor_somds_dec_gateway>> shall assign the handles (which are required to be unique in the same <<acronym_mdib>> sequence) of the containment tree elements representing MDSs, VMDs, channels and metrics to unique integer numbers per child level of the same parent.
...
****
A <<actor_somds_dec_gateway>> shall define the measurement range of the numeric metric measurement channel by the format `<Lower Limit> - <Upper Limit>` where
...
****
If a <<actor_somds_dec_gateway>> is intended to export metrics with a different validity state (*pm:AbstractMetricValue/pm:MetricQuality/@Validity*) than valid or validated, the measurement status shall be set in this field as specified in the <<ihe_pcd_tf_2_2019>>.
****
...
NOTE: The <<ihe_pcd_tf_2_2019>> requires only the map the "ID Number", and optionally the "Family Name" and "Given Name" in this field, whereas the OBR-10 field may contain additional information about the operator.
====
****
