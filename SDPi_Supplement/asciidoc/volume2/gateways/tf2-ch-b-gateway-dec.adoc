...
A <<actor_somds_dec_gateway>> shall set the OBR-7 field to the date and time of all continuously (periodically) measured metrics reported by a *msg:PeriodicMetricReport* to be exported within the same *msg:RetrievabilityInfo/@UpdatePeriod*.

.Notes
[%collapsible]
====
NOTE: If, for example, the *@UpdatePeriod* is set to one second, a *msg:PeriodicMetricReport* is expected every second with new periodic metric states to be exported within this one second interval. The OBR-7 field is set to the start time of the interval. The individual periodic metric *@DeterminationTime* is basically ignored, but has to be within the time boundaries of the current export interval.
...
For episodic measurements and the absence of any continuously measured metrics for this export interval, a <<actor_somds_dec_gateway>> shall set the OBR-7 field to the oldest measurement timestamp of the episodic metrics to be exported in the current export interval.

.Notes
[%collapsible]
====
NOTE: Episodic metric measurements are usually exported along with the periodic metric measurements in the same export intervals triggered by *msg:PeriodicMetricReport* events. However, if a device does not provide periodic metric measurements, or if the *@UpdatePeriod* is too long, episodic metric measurements are exported independently of the periodic metric measurements in separate HL7 data export messages.
...
/pm:MetricValue
/@DeterminationTime
|If *pm:AbstractMetricDescriptor /@MetricAvailability* is set to "*Cont*", the field is set to the start time of the *msg:PeriodicMetricReport* event. The *@DeterminationTime* of the *pm:AbstractMetricState* of continuously measured metrics to be exported in the same export interval has to be within the start time of the *msg:PeriodicMetricReport* event and the *msg:RetrievabilityInfo /@UpdatePeriod*.

New *pm:AbstractMetricState* elements of episodically measured metrics are exported in the same HL7 message with the *@DeterminationTime* set in HL7 OBX-14 field overriding the time point in the OBR-7 field.
...
****
If a <<actor_somds_dec_gateway>> is intended to export metrics with a different validity state (*pm:AbstractMetricValue/pm:MetricQuality/@Validity*) than valid or validated, the measurement status shall be set in this field as specified in the <<ref_ihe_pcd_tf_2_2019>>.
****
...
In all other cases, the field is set to the pm:AbstractMetricState /pm:BodySite /@SymbolicCodeName.
| Note that MDC is the default coding system if no coding system is specified.
...
In all other cases, the field is set to pm:AbstractMetricState /pm:BodySite /@CodingSystem.
