...
The OBX-5 field shall contain the value of the SDC metric. The formatting of the data depends on the data type.

For a device-releted element such as MDS, VMD, or CHANNEL, the OBX-5 field shall be left empty.

[NOTE]
====
Only metrics with a *pm:AbstractMetricValue/pm:MetricQuality/@Validity* set to *Vld* (Valid) or *Vldated* (Validated Data) shall be exported.

Metrics with a different *@Validity* shall be skipped/ignored.
====

[NOTE]
====
Only metrics with the *pm:AbstractMetricDescriptor/@MetricCategory* set to *Msrmt* (Measurement), *Clc* (Calculation) or *Set* (Setting) shall be exported.

Metrics with a different *@MetricCategory* shall be skipped/ignored.
====

//Todo: need another sub-level for the headline
====== Numeric Metric
For a numeric metric, the OBX-5 field shall be set to the *pm:NumericMetricState/pm:MetricValue/@Value*.

[NOTE]
====
The decimal number shall be formatted according the HL7 numeric value formatting rules.
====

//Todo: need another sub-level for the headline
====== String Metric
For a string metric, the OBX-5 field shall be set to the *pm:StringMetricState/pm:MetricValue/@Value*.

//Todo: need another sub-level for the headline
====== Enumeration String Metric
For an enumeration string metric, the OBX-5 field shall be set to coded element value. The table <<ref_tbl_dec_obx5_enum_mapping>> below defines the mapping of the SDC coded element value to the data fields of the HL7 data type *CWE* used in the OBX-5 field.

The *pm:EnumStringMetricState/pm:MetricValue/@Value* contains the string of the selected enumerated element. The actual coded element value has to be retrieved from the *pm:EnumStringMetricDescriptor/pm:AllowedValue* list by comparing the *pm:EnumStringMetricState/pm:MetricValue/@Value* with the *pm:EnumStringMetricDescriptor/pm:AllowedValue/pm:Value*.

When a match has been found, the *pm:EnumStringMetricDescriptor/pm:AllowedValue/pm:Type* shall be mapped as defined in table <<ref_tbl_dec_obx5_enum_mapping>> below.

If no matching value could be found, the *pm:EnumStringMetricState/pm:MetricValue/@Value* shall be set in the OBX-5 field and OBX-2 shall be set to *"ST"* (see also <<ref_tbl_dec_obx2_mapping>>).
...
In all other cases, the field shall be set to the pm:EnumStringMetricDescriptor /pm:AllowedValue /pm:Type /@SymbolicCodeName.
| Note that MDC is the default coding system if no coding system is specified.
...
In all other cases, the field shall be set to pm:EnumStringMetricDescriptor /pm:AllowedValue /pm:Type /@CodingSystem.
...
====== OBX-6 Units
For a numeric metric, the OBX-6 field shall be set to measurement unit. The table <<ref_tbl_dec_obx6_mapping>> below defines the mapping of the SDC measurement unit to the data fields of the HL7 data type *CWE* used in the OBX-6 field.

For a device-releted element such as MDS, VMD, CHANNEL, or other metric types, the OBX-6 field shall be left empty.

[NOTE]
====
If a private *MDC* code is used for the coding of the SDC measurement unit, the identifier shall be mapped as described in section <<ref_private_coding_system_note>>.
====
...
In all other cases, the field shall be set to the pm:NumericMetricDescriptor /pm:Unit /@SymbolicCodeName.
| Note that MDC is the default coding system if no coding system is specified.
...
In all other cases, the field shall be set to pm:NumericMetricDescriptor /pm:Unit /@CodingSystem.
...
The OBX-7 field shall define the measurement range of the numeric metric measurement channel - not on the metric level.
The format of the range string shall be *"<Lower Limit> - <Upper Limit>"* where

* <Lower Limit> is set to *pm:NumericMetricDescriptor/pm:TechnicalRange/@Lower*
* <Upper Limit> is set to *pm:NumericMetricDescriptor/pm:TechnicalRange/@Upper*

The OBX-7 field shall define the range of the alert limits on the metric level, when the *@Handle* of the metric is referenced by a *pm:LimitAlertConditionDescriptor* in the *pm:LimitAlertConditionDescriptor\pm:Source* list. The format of the range string shall be *"<Lower Limit> - <Upper Limit>"* where

* <Lower Limit> is set to *pm:LimitAlertConditionDescriptor/pm:MaxLimits/@Lower*
...
If there is a need to export also metrics with a different validity state (*pm:AbstractMetricValue/pm:MetricQuality/@Validity*), the measurement status must be set in this field as specified in the <<ihe_pcd_tf-2_2019>>.

====== OBX-11 Observation Result Status
For a device-releted element such as MDS, VMD, or CHANNEL, the OBX-11 field shall be set to *"X"*.

For metrics with the *pm:AbstractMetricValue/pm:MetricQuality/@Validity* set to *Vld* (Valid), the OBX-11 field shall be set to *"R"*.

For metrics with the *pm:AbstractMetricValue/pm:MetricQuality/@Validity* set to *Vldated* (Validated Data), the OBX-11 field shall be set to *"F"*.

====== OBX-14 Date/Time of Observation
The OBX-14 field shall contain the date & time of the intermittently measured metric value.
Intermittently measured metrics have the *pm:AbstractMetricDescriptor/@MetricAvailability* is set to "*Intr*".
The table <<ref_tbl_dec_obx14_mapping>> defines the mapping of the SDC metric measurement timestamp to the data fields of the HL7 data type *DTM* used in the OBX-14 field.
...
The operator shall be defined in the OBR-10 field. Please refer to <<ref_dec_obr10>> for further information.

====== OBX-17 Observation Method
The OBX-17 field shall be set to one of the coded terms as specified in table <<ref_tbl_dec_obx17_mapping>> below dependent on the *pm:AbstractMetricDescriptor/@MetricCategory* (Category) and the *pm:AbstractMetricDescriptor/@DerivationMethod* (Derivation).
...
|Msmt
|Auto
|OBX-17 field shall be left empty (recommended).
...
The OBX-20 field shall be set to body site if available for the metric. The table <<ref_tbl_dec_obx20_mapping>> below defines the mapping of the SDC body site element to the data fields of the HL7 data type *CWE* used in the OBX-6 field.

For a device-releted element such as MDS, VMD, or CHANNEL, the OBX-20 field shall be left empty.
...
In all other cases, the field shall be set to the pm:AbstractMetricState /pm:Unit /@SymbolicCodeName.
| Note that MDC is the default coding system if no coding system is specified.
...
In all other cases, the field shall be set to pm:AbstractMetricState /pm:Unit /@CodingSystem.
