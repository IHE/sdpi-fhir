...
In all other cases, the field is set to the pm:xxxMetricDescriptor+++<wbr/>+++/pm:Type+++<wbr/>+++/@SymbolicCodeName.
|Note that MDC is the default coding system if no coding system is specified.
...
In all other cases, the field is set to pm:xxxMetricDescriptor+++<wbr/>+++/pm:Type+++<wbr/>+++/@CodingSystem.
