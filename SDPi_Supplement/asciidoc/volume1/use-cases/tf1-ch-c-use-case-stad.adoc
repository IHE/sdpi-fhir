...
===== Scenario: STAD {var_use_case_id}.1 - Device is connected to the MD LAN network with a Time Source service
...
===== Scenario: STAD {var_use_case_id}.2 - Device is connected to the MD LAN network with a TS Service and a user wants to change the device's time
...
===== Scenario: STAD {var_use_case_id}.3 - Device is connected to the MD LAN network and cannot connect to a TS Service
...
===== Scenario: STAD {var_use_case_id}.4 - Devices are connected to the MD LAN network but cannot connect to a TS Service and a device’s time setting is changed manually
