...
[#vol1_clause_soa_somds_architecture_alignment]
...
The <<term_medical_data_information_base>> component applies to all participating systems and consists of a *_descriptive model_* (e.g., what services and information a <<term_somds_provider>> supports), and a state model.
...
This profile fully addresses the requirements from  <<vol1_appendix_c_use_case_stad>>.

#TODO:  Add discussion to HOW and WHERE (with links) that the requirements are met.  Include a Requirements usage block?  CT DEPENDENCY IN SOMDS_PARTICIPANT MEETS THESE REQUIREMENTS#

===== <add additional use case links here>
