...
One of the key challenges for implementing this standard, though, is what might be labeld:  *_The Interoperability Trust Gap_*.
...
[none]
.. Additional non-PKP risk management will also be performed by subject matter experts and formalized in these <<acronym_ses>> consideration sections, where appropriate.

These SES Considerations sections grew out of the _IHE "Security Considerations"_ sections + the IHE Devices _"Safety Considerations"_ sections, but are now consolidated into a single SES Considerations section that integrates the 3rd risk management property, Effectiveness.
...
| SDPi Requirement
| A defined stakeholder-imposed constraints that must be satisfied for a design solution to be valid.  This is an {abstract} element.
| sdpi_requirement
| See subtypes
